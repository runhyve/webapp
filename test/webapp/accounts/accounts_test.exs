defmodule Webapp.AccountsTest do
  ...
  describe "ssh_public_keys" do
    ...
    def ssh_public_key_fixture(attrs \\ %{}) do
      ...
      {:ok, ssh_public_key} = Accounts.create_ssh_public_key(user, Enum.into(attrs, @create_attrs))
    ...
    test "create_ssh_public_key/1 with valid data creates a ssh_public_key" do
      %{team: team, user: user} = user_fixture(@user1)
      assert {:ok, %SSHPublicKey{} = ssh_public_key} = Accounts.create_ssh_public_key(user, @create_attrs)
      assert ssh_public_key.ssh_public_key == "ssh-rsa  dummy@key"
