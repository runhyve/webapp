defmodule Webapp.DistributionsTest do
  ...
  describe "distributions" do
    ...
    @valid_attrs %{name: "FreeBSD", version: "12.0", loader: "bhyveload", image: "http://ftp.icm.edu.pl/pub/FreeBSD/releases/VM-IMAGES/12.0-RELEASE/amd64/Latest/FreeBSD-12.0-RELEASE-amd64.raw.xz"}
    @update_attrs %{name: "Ubuntu", version: "18.04", loader: "grub", image: "https://cloud-images.ubuntu.com/releases/bionic/release/ubuntu-18.04-server-cloudimg-amd64-uefi1.img"}
    ...
    test "get_img_name/1 returns valid file name" do
      ...
      assert {:ok, %Distribution{} = distribution} = Distributions.update_distribution(distribution, @update_attrs)
      assert Distributions.get_img_name(distribution) == "ubuntu-18.04-server-cloudimg-amd64-uefi1.img"
    end

    test "create_distribution/1 with valid data creates a distribution" do
      assert {:ok, %Distribution{} = distribution} = Distributions.create_distribution(@valid_attrs)
      assert distribution.name == "FreeBSD"
      assert distribution.version == "12.0"
      assert distribution.loader == "bhyveload"
      assert distribution.image == "http://ftp.icm.edu.pl/pub/FreeBSD/releases/VM-IMAGES/12.0-RELEASE/amd64/Latest/FreeBSD-12.0-RELEASE-amd64.raw.xz"
    end
    ...
    test "update_distribution/2 with valid data updates the distribution" do
      ...
      assert distribution.image == "https://cloud-images.ubuntu.com/releases/bionic/release/ubuntu-18.04-server-cloudimg-amd64-uefi1.img"
    end

    test "update_distribution/2 with invalid data returns error changeset" do
      distribution = distribution_fixture()
      assert {:error, %Ecto.Changeset{}} = Distributions.update_distribution(distribution, @invalid_attrs)
